...
A single line in the diff output. Fields: `kind` ("equal" | "insert" | "delete"), `text` (content without trailing newline), `oldRow?` (0-based old buffer row; undefined for inserts), `newRow?` (0-based new buffer row; undefined for deletes).

#### 4.1.2 DiffHunk

A contiguous group of diff lines with shared context, analogous to a unified diff hunk. Carries `oldStart`, `oldCount`, `newStart`, `newCount`, and `lines` (including context lines).
...
Visual styling applied to a range of text. Fields: `range` (MultiBufferRange) and `style` (Partial\<DecorationStyle\>).

#### 4.1.5 DecorationStyle

All visual properties for a decorated line. Text fields: `backgroundColor`, `color`, `borderColor`, `fontWeight`, `fontStyle`, `textDecoration`. Gutter fields: `gutterBackground`, `gutterColor`, `gutterSign`, `gutterSignColor`. All are optional.

#### 4.1.6 DiffController

Controller for a diff view with re-diff on edit support. See §7.2 for the full TypeScript interface. Properties: `multiBuffer`, `decorations`, `isEqual`, `oldBuffer`, `newBuffer`. Methods: `reDiff()`, `notifyChange()`, `onUpdate(callback)`, `dispose()`.
...
**Process**: Cancel any pending timer, schedule new re-diff after debounce. On fire: read both buffers, run `diff()`, rebuild excerpts from the result, regenerate decorations, notify subscribers.
