...
A single line: `kind` ("equal" | "insert" | "delete"), `text` (content without trailing newline), `oldRow` (0-based in old buffer; undefined for inserts), `newRow` (0-based in new buffer; undefined for deletes).

#### 4.1.2 DiffHunk

A contiguous group of diff lines with shared context, analogous to a unified diff hunk. Fields: `oldStart`, `oldCount`, `newStart`, `newCount` (line number ranges in each buffer), and `lines` (`readonly DiffLine[]` including context).

#### 4.1.3 DiffResult

Complete diff output: `hunks` (`readonly DiffHunk[]`) and `isEqual` (true when texts are identical).

#### 4.1.4 Decoration

Visual styling applied to a `range` (MultiBufferRange) with a `style` (Partial<DecorationStyle>): backgroundColor, gutterSign, gutterSignColor, etc.

#### 4.1.5 DecorationStyle

All visual properties for a decorated line. Text properties: `backgroundColor`, `color`, `borderColor`, `fontWeight`, `fontStyle`, `textDecoration`. Gutter properties: `gutterBackground`, `gutterColor`, `gutterSign` (e.g., "+" or "−"), `gutterSignColor`.

#### 4.1.6 DiffController

Controller for a live diff view. Properties: `multiBuffer`, `decorations`, `isEqual`, `oldBuffer`, `newBuffer`. Methods: `reDiff()`, `notifyChange()`, `onUpdate(callback)`, `dispose()`. See §7.2 for the full TypeScript interface.
...
**Process**: Cancel any pending timer, schedule a new re-diff after the debounce delay. On fire: read both buffers, run `diff()`, replace all excerpts, regenerate decorations, notify subscribers.
...
The MultiBuffer's anchor system handles cursor preservation. The editor records cursor anchors (excerpt ID + buffer offset + version) before re-diff. When excerpts are rebuilt, the replacement map tracks old→new ID mappings; `resolveAnchor()` follows the chain to restore the cursor position.
