...
A single line in the diff output. `kind` classifies the change ("equal" | "insert" | "delete"); `text` holds the content without trailing newline; `oldRow` is the 0-based line number in the old buffer (undefined for inserts); `newRow` is the 0-based line number in the new buffer (undefined for deletes).

#### 4.1.2 DiffHunk

A contiguous group of diff lines with shared context, analogous to a unified diff hunk. Fields: `oldStart`/`oldCount` (start and line count in old buffer), `newStart`/`newCount` (start and line count in new buffer), `lines` (the hunk's lines including context).

#### 4.1.3 DiffResult

Complete diff output: `hunks` lists all change hunks; `isEqual` is true when old and new texts are identical.

#### 4.1.4 Decoration

Visual styling applied to a range of text. `range` (MultiBufferRange) identifies the rows; `style` (Partial<DecorationStyle>) specifies visual properties such as backgroundColor, gutterSign, and gutterSignColor.

#### 4.1.5 DecorationStyle

All visual properties for a decorated line. Content fields: `backgroundColor`, `color`, `borderColor`, `fontWeight` ("normal" | "bold"), `fontStyle` ("normal" | "italic"), `textDecoration` ("none" | "underline" | "line-through"). Gutter fields: `gutterBackground`, `gutterColor`, `gutterSign` (e.g., "+" or "−"), `gutterSignColor`.

#### 4.1.6 DiffController

Controller for a diff view with re-diff on edit support. Exposes `multiBuffer`, `decorations`, `isEqual`, `oldBuffer`, `newBuffer`, plus methods `reDiff()`, `notifyChange()`, `onUpdate()`, and `dispose()`. See §7.2 for the full TypeScript interface.
...
The MultiBuffer's anchor system handles cursor preservation. Before each re-diff, the editor creates anchors referencing excerpt ID, buffer offset, and version. When excerpts are rebuilt, the replacement chain maps old→new IDs; `resolveAnchor()` follows the chain and adjusts for buffer edits to restore the cursor position.
