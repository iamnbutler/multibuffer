...
Status: Draft v1 — implemented in [`src/diff/`](../src/diff/). Where this spec and the code disagree, the code is authoritative.
...
The system must interleave deleted lines from the old version with inserted and modified lines from the new version, allow editing of the new-buffer lines while keeping delete lines read-only, update the diff live as those edits land, and preserve the cursor across recalculations. See §2.1 for the itemized goals.
...
|-----------|--------|------|
| Diff algorithm | `src/diff/diff.ts` | Myers' O(ND) line-level diff; groups edits into hunks with configurable context; returns `DiffResult` (§4.1.3). Also computes intraline diffs (§5.8). |
...
| Controller | `src/diff/controller.ts` | Wraps the builder with debounced re-diff and update subscriptions (§5.4). |
| Diff gutter renderer | `src/renderer/dom.ts` | Renders the dual line-number gutter and applies decoration styles when `gutterMode: "diff"` (§4.3, §6.2). |
| Editor | `src/editor/editor.ts` | Dispatches edits, honors the excerpt `editable` flag, fires `onChange` (§5.3). |

`src/diff/` also contains modules outside this spec's scope: `patch.ts` (unified-diff patch parsing), `multi-file.ts` (multi-file diff views), `unified.ts` (flat line list without a MultiBuffer), and `diff-editor-view.ts` (high-level facade).
...
An edit flows from `MultiBuffer.edit()` through the editor's `onChange` callback to the controller's `notifyChange()`, which schedules the debounced rebuild detailed in §5.4.
...
A contiguous group of diff lines with shared context. Analogous to a unified diff hunk (`@@ -a,b +c,d @@`).
...
Visual styling applied to a range of text. `range` (`MultiBufferRange`) is the region styled — full rows for line decorations, a column span for intraline highlights (§5.8) — and `style` (`Partial<DecorationStyle>`) carries the visual properties.

#### 4.1.5 DecorationStyle

Text styling uses `backgroundColor`, `color`, `borderColor`, `fontWeight` (`"normal" | "bold"`), `fontStyle` (`"normal" | "italic"`), and `textDecoration` (`"none" | "underline" | "line-through"`). Gutter styling uses `gutterBackground`, `gutterColor`, `gutterSign` (the sign character, e.g. `"+"` or `"−"`), and `gutterSignColor`. All are `string` unless noted.

The optional `isHunkSeparator` (`boolean`) marks a hunk separator line (§6.3): the gutter spans the full width and no line numbers are drawn.
...
**Standard mode** (`gutterMode: undefined | "standard"`) draws a single gutter column with the MultiBuffer row number, `gutterWidth` wide, from `Measurements`.

**Diff mode** (`gutterMode: "diff"`) draws three fixed-width columns — old line number (40px), new line number (40px), sign (16px) — for an effective gutter width of 96px. This total is the reference for all layout and hit-testing math below.
...
**Divergence** (edit makes equal differ from old): the equal excerpt splits into a delete (from old) + insert (from new) pair, and the line count increases.
...
Delete and insert lines that pair up within a hunk also receive character-level decorations, so a one-word change highlights that word rather than the whole line. `computeIntralineDiff()` returns the changed column ranges for each side of the pair; those become column-range decorations using the same hues at stronger opacity (`rgba(255, 80, 80, 0.25)` for deletes, `rgba(80, 200, 80, 0.25)` for inserts). Enabled by default; disable with `intraline: false`.
...
Both line numbers are right-aligned with 4px right padding; the sign is centered; the content column takes the remaining width and scrolls horizontally when needed.

### 6.3 Hunk Separators

When a diff has multiple non-adjacent hunks, a separator line is inserted between them carrying the `@@ -X,Y +A,B @@` header (plus function/class context when available). Separator lines live in their own buffer — held by `separatorBuffer` on the result, which callers must keep referenced — and are styled with `HUNK_HEADER_STYLE`, whose `isHunkSeparator: true` tells the renderer to draw a full-width gutter with no line numbers. Enabled by default; disable with `showHunkSeparators: false`.

### 6.4 Hit Testing and Selection

Both must use the 96px effective gutter width from §4.3, not `gutterWidth`. Content starts at x = 96px, so `hitTest(x, y)` computes the column as `(x - 96) / charWidth` and a selection rectangle starts at `96 + (startColumn * charWidth)`, spanning the selected column range.
...
interface UnifiedDiffMultiBufferResult {
  ...
  /** Buffer holding hunk separator text lines; keep referenced to prevent GC. */
...
interface DiffControllerOptions extends DiffOptions, UnifiedDiffMultiBufferOptions {
  ...
  debounceMs?: number;
  /** Make all excerpts non-editable, for read-only viewers. Default: false. */
  readOnly?: boolean;
}
...
Hunk folding (needs excerpt expand/collapse support) and three-way merge (needs a significantly different architecture) are both out of scope for v1.

Undo is **not** deferred: the editor's undo stack operates on the new buffer independently of diff state.

Character-level highlighting was originally deferred but has since shipped — see §5.8.
