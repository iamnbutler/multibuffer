...
|-----------|--------|------|
| Diff Algorithm | `src/diff/diff.ts` | Myers' O(ND) line-level diff, grouped into hunks with configurable context (§5.1). Returns `DiffResult`. |
| Diff MultiBuffer Builder | `src/diff/multibuffer.ts` | Diffs old/new `Buffer` text and builds a `MultiBuffer` with excerpts from the appropriate source buffers plus `Decoration[]` (§5.2). |
| Diff Controller | `src/diff/controller.ts` | Wraps the MultiBuffer with debounced change detection and re-diff, notifying subscribers on decoration updates (§5.4). |
| Diff Gutter Renderer | `src/renderer/dom.ts` | Renders dual line-number columns and signs when `gutterMode: "diff"` (§4.3, §6.2). |
| Editor | `src/editor/editor.ts` | Handles editing, respects the `editable` flag, and fires `onChange` after mutations (§5.3). |
...
| `style` | Partial<DecorationStyle> | Visual properties (backgroundColor, gutterSign, etc.). |
...
Controller for a diff view with re-diff on edit support. See the `DiffController` interface in §7.2 for the full member list.
...
In diff mode, `hitTest(x, y)` uses the 96px diff gutter width (§6.2): content starts at x = 96px, so column = `(x - 96) / charWidth`.

### 6.4 Selection Rendering

Selection rectangles use the same 96px offset: x-start = `96 + startColumn * charWidth`, with width spanning the selected column range.
