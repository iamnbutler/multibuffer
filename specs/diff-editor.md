...
| Diff MultiBuffer Builder | `src/diff/multibuffer.ts` | Diffs old/new `Buffer` text and builds a `MultiBuffer` of source excerpts plus `Decoration[]` (§5.2). |
| Diff Controller | `src/diff/controller.ts` | Wraps the diff MultiBuffer with debounced change detection and decoration-update notifications via `notifyChange()` (§5.4, §7.2). |
...
| Editor | `src/editor/editor.ts` | Existing editor; respects excerpt `editable` flags and fires `onChange` after mutations (§5.3). |
