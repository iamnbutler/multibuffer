...
| Diff MultiBuffer Builder | `src/diff/multibuffer.ts` | Builds a `MultiBuffer` of excerpts from old/new buffers and generates `Decoration[]` (§5.2). |
| Diff Controller | `src/diff/controller.ts` | Wraps the diff MultiBuffer with debounced change detection and re-diff (§5.4, §7.2). |
...
|-------|------|-------------|
| `kind` | "equal" \| "insert" \| "delete" | Type of change this line represents. |
...
| `lines` | readonly DiffLine[] | Lines in this hunk, including context. |
...
| Field | Type | Description |
|-------|------|-------------|
| `range` | MultiBufferRange | Rows this decoration applies to. |
...
Controller for a diff view with re-diff-on-edit support. See the full `DiffController` interface and its members in §7.2.
