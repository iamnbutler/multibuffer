 ...
 *
 * Language-specific node type → category mappings live in queries/:
 *   queries/typescript.ts  - TypeScript/JavaScript node types
 *   queries/markdown.ts    - Markdown node types
 *   queries/yaml.ts        - YAML node types
 *
 * This file owns the category → color mapping (theming concern).
 *
...
import { MARKDOWN_NODE_CATEGORIES } from "./queries/markdown.ts";
import { TYPESCRIPT_NODE_CATEGORIES } from "./queries/typescript.ts";
import { YAML_NODE_CATEGORIES } from "./queries/yaml.ts";
 ...
 * Built by merging the per-language query maps from queries/.
 * O(1) lookup; note that "}" maps to "string" (template literal close)
 * rather than "punctuation", matching tree-sitter's token representation.
 */
const NODE_TYPE_CATEGORY: ReadonlyMap<string, string> = new Map([
  ...TYPESCRIPT_NODE_CATEGORIES,
  ...MARKDOWN_NODE_CATEGORIES,
  ...YAML_NODE_CATEGORIES,
]);
