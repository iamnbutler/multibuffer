...
The type of change a [DiffLine](#diffline) represents: `"equal"` — unchanged between old and new; `"insert"` — added in the new version (no `oldRow`); `"delete"` — removed from the old version (no `newRow`).
