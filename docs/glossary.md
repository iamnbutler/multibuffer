...
A pure function (`src/buffer/offset.ts`) that advances a `BufferOffset` through a chronological sequence of `EditEntry` values. Applies `adjustOffsetSingle` for each edit in turn, respecting [Bias](#bias) at edit boundaries: offsets before the edit pass through unchanged; offsets after the deleted range shift by `insertedLength − deletedLength`; at the edit start, `Bias.Right` jumps past inserted text while `Bias.Left` (or any position within the deleted range) clamps to the edit start. Used by multibuffer anchor resolution when replaying edits since an anchor's recorded version.
...
An automated PR triage agent (`.github/workflows/closer.md`). Runs after a review is submitted and decides each PR's outcome: labels `ready-to-merge` when CI is green with no blocking reviews, labels `needs-review` when blocking reviews or unresolved issues exist, and closes PRs that are duplicate, spam, or fundamentally broken. Defaults to `needs-review` when uncertain. Chains naturally after the [Reviewer](#reviewer); draft status is irrelevant — all PRs are triaged on their code and review state alone.

See: `.github/workflows/closer.md`

### Coordinate Systems

The project uses two distinct coordinate spaces: **buffer coordinates** are a row/column or byte offset within a single source file (`BufferRow`, `BufferOffset`, `BufferPoint`), while **multibuffer coordinates** are the same within the unified scrollable view across all excerpts (`MultiBufferRow`, `MultiBufferOffset`, `MultiBufferPoint`). Branded types enforce that these are never accidentally mixed.
...
The behavior of `clipPoint` and `clipOffset` (`src/buffer/buffer.ts`) when a clamped position lands inside a UTF-16 surrogate pair (e.g., emoji or other supplementary Unicode characters outside the Basic Multilingual Plane). A position is inside a surrogate pair when it points at a low surrogate (code unit 0xDC00–0xDFFF); [Bias](#bias) then determines the snap direction: `Bias.Left` steps back to the high surrogate (the position *before* the supplementary character), and `Bias.Right` steps past the low surrogate (the position *after* it). This matches the surrogate-pair-aware cursor movement in `cursor.ts`, which uses `codePointAt`/`prevCpStart` helpers to traverse pairs atomically.
