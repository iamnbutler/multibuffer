...
A stable position within a buffer or multibuffer that survives text edits. Anchors track an offset and a [bias](#bias), and are updated by replaying the buffer's edit log when the buffer changes. Used to represent cursor positions and selection endpoints durably.
...
A pure function (`src/buffer/offset.ts`) that advances a `BufferOffset` through a chronological sequence of `EditEntry` values, applying `adjustOffsetSingle` to each in turn. Offsets before an edit pass through unchanged; offsets after the deleted range shift by `insertedLength − deletedLength`. At the edit start, [Bias](#bias) resolves the ambiguity: `Bias.Right` jumps past the inserted text, while `Bias.Left` — and any offset inside the deleted range — clamps to the edit start.
...
Inheriting the current line's leading whitespace on a newly inserted line. Implemented only by the `insertLineBelow` and `insertLineAbove` commands, which copy the cursor line's leading spaces (matched by `/^( +)/`) and place the cursor after the indentation.

Note that `insertNewline` does **not** auto-indent — it inserts a bare `\n`.
...
A hint controlling behavior at position boundaries — when text is inserted at an anchor's offset, or when a point is clipped to valid bounds. `Bias.Left` stays left of inserted text and clips to the position before a boundary; `Bias.Right` advances past inserted text and clips to the position at or after it.
...
A branded offset within a single buffer, measured in UTF-16 code units (see [Coordinate Systems](#coordinate-systems)). Distinct from [MultiBufferOffset](#multibufferoffset) to prevent mixing coordinate systems at compile time.
...
An automated PR triage agent (`.github/workflows/closer.md`) that runs after a review is submitted and decides each pull request's outcome: the `ready-to-merge` label when CI is green and no blocking reviews remain, `needs-review` when blocking reviews or unresolved issues exist, and closure for PRs that are duplicate, spam, or fundamentally broken. Defaults to `needs-review` when uncertain, and chains naturally after the [Reviewer](#reviewer). Draft status is irrelevant — all PRs are triaged on their code and review state alone.

See: `.github/workflows/closer.md`

### Coordinate Systems

The project uses two distinct coordinate spaces, kept apart by branded types so they can never be accidentally mixed:

- **Buffer coordinates** — a position within a single source file (`BufferRow`, `BufferOffset`, `BufferPoint`).
- **Multibuffer coordinates** — a position within the unified scrollable view across all excerpts (`MultiBufferRow`, `MultiBufferOffset`, `MultiBufferPoint`).

In both spaces, all offsets, lengths, and column values are measured in **UTF-16 code units** — never bytes. A supplementary code point such as an emoji counts as 2 units. The one genuinely byte-valued field in the project is [`TextSummary.bytes`](#textsummary), which is for display only and must not be used for position arithmetic.
...
A visual annotation applied to a range of text in the renderer. Each decoration specifies a `MultiBufferRange` and optionally a CSS `className` and a `Partial<DecorationStyle>`. The DOM renderer builds a per-row lookup during each render pass; when two decorations overlap the same row, the later entry in the array wins.

### DecorationStyle

An interface (`src/renderer/types.ts`) describing the full set of visual properties a [Decoration](#decoration) can apply to a row: `backgroundColor`, `color`, `borderColor`, `fontWeight`, `fontStyle`, `textDecoration`, the gutter-specific `gutterBackground`, `gutterColor`, `gutterSign`, and `gutterSignColor`, plus `isHunkSeparator` — which makes the gutter span the full width with no line number, used for hunk separator lines in diff view.
...
The type of change a [DiffLine](#diffline) represents: `"equal"` (unchanged between old and new), `"insert"` (added in the new version, so it has no `oldRow`), or `"delete"` (removed from the old version, so it has no `newRow`).

### DiffLine

A single line in a diff result, carrying its [DiffKind](#diffkind), `text`, and source row numbers — `oldRow` from the old buffer and `newRow` from the new buffer.
...
The specification for creating an excerpt: a `context` range — the full `BufferRange` to display, including any surrounding context lines — and `primary`, the highlighted sub-range within it.
...
The allocation strategy behind [SlotMap](#slotmap). Each slot carries a generation counter that increments on reuse, making stale keys detectable in O(1) without call-site bookkeeping.

### Goal Column

A remembered column position stored by the `Editor` for vertical cursor navigation (`moveUp`, `moveDown`). When moving through lines of unequal length, the cursor targets the goal column rather than the current line's actual column, so it returns to its original column after passing through shorter intermediate lines. Cleared by horizontal movement or any edit, and reset at the start of each new vertical movement.
...
> Its instructions delegate non-trivial CI fixes by commenting `/pr-fix` on the PR, but no `pr-fix` workflow currently exists in `.github/workflows/`, so those comments are inert.
...
An optimization in `Highlighter.parseBuffer()` where the previous parse tree is passed to tree-sitter's `parser.parse(text, oldTree)`, so unchanged subtrees are reused instead of re-parsing the entire file on every edit. Enabled by supplying a [TreeEdit](#treeedit) descriptor so the old tree is updated via `tree.edit()` before re-parsing; on large files this eliminates significant per-keystroke overhead.

See: `src/renderer/highlighter.ts`, [TreeEdit](#treeedit)

### InputHandler

A class (`src/editor/input-handler.ts`) that captures keyboard input via a hidden off-screen `<textarea>` element — a textarea rather than raw `keydown` listeners, so IME (Input Method Editor) composition works for CJK and other complex scripts. On each keyboard event it calls [keyEventToCommand](#keyeventtocommand) to produce an `EditorCommand`; if no command matches, the `input` event carries the typed text instead. Exposes `mount(container)`, `unmount()`, `focus()`, and `blur()`.
...
A branded offset within the multibuffer's unified view, measured in UTF-16 code units (see [Coordinate Systems](#coordinate-systems)). Distinct from [BufferOffset](#bufferoffset).
...
An immutable snapshot of the multibuffer's state, supporting read operations (`lines`, `excerptAt`, `toBufferPoint`, `toMultiBufferPoint`, `resolveAnchor`, `resolveAnchors`, `clipPoint`, `excerptBoundaries`) without mutation concerns. Carries a monotonically increasing `version` counter that increments on every mutation, shared globally across all `MultiBuffer` instances; the DOM renderer uses it to skip [WrapMap](#wrapmap) reconstruction when neither the snapshot content nor the wrap width has changed since the last render.
...
An array where entry `i` holds the cumulative total of entries `0..i`. Used by [Rope](#rope) (chunk start offsets) and [WrapMap](#wrapmap) (visual row offsets) for O(1) forward lookup and O(log n) reverse lookup via binary search.
...
A mode in which the `Editor` silently ignores all text-mutating commands — including `insertText`, `cut`, `redo`, `deleteLine`, `moveLine`, `duplicateLine`, `indentLines`, and `dedentLines` — while still processing cursor-movement commands. Enabled by passing `readOnly: true` to the `Editor` constructor or calling `editor.setReadOnly(true)` at runtime; the current state is readable via the `editor.readOnly` getter.
...
The text storage structure backing each [buffer](#buffer). Splits text into fixed-size chunks (at most 1024 UTF-16 code units, preferring newline boundaries); insert/delete/replace return new Rope instances with structural sharing of unchanged chunks. Caches chunk start offsets as a prefix-sum array for O(log n) line↔offset conversion.
...
The behavior of `clipPoint` and `clipOffset` (`src/buffer/buffer.ts`) when a clamped position lands inside a UTF-16 surrogate pair — an emoji or other supplementary character outside the Basic Multilingual Plane. A position is inside a surrogate pair when it points at a low surrogate (code unit 0xDC00–0xDFFF); [Bias](#bias) then decides the snap direction, `Bias.Left` stepping back to the high surrogate (the position *before* the character) and `Bias.Right` stepping past the low surrogate (the position *after* it).
...
Cached aggregate metrics for a span of text, stored per-excerpt to enable O(1) position lookups without scanning: `lines`, `lastLineLength`, and `chars` (all in UTF-16 code units), plus `bytes` — the UTF-8 encoded byte count, which is for display only and must never be used for position arithmetic.

See: `src/buffer/types.ts`, [Coordinate Systems](#coordinate-systems)
