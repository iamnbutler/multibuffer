...
A pure function (`src/buffer/offset.ts`) that advances a `BufferOffset` through a chronological sequence of `EditEntry` values. Applies `adjustOffsetSingle` for each edit in turn, respecting [Bias](#bias) at edit boundaries: offsets before the edit pass through unchanged; offsets after the deleted range shift by `insertedLength − deletedLength`; offsets at the edit start with `Bias.Right` jump past inserted text, while `Bias.Left` (or positions within the deleted range) clamp to the edit start. Used by multibuffer anchor resolution when replaying edits since an anchor's recorded version.
...
The type of change a [DiffLine](#diffline) represents: `"equal"` (line unchanged between old and new), `"insert"` (added in the new version, no `oldRow`), or `"delete"` (removed from the old version, no `newRow`).
...
The specification for creating an excerpt: `context` (the full `BufferRange` to display, including surrounding context lines) and `primary` (the highlighted sub-range within `context`).
