...
The project uses two distinct coordinate spaces: **buffer coordinates** (`BufferRow`, `BufferOffset`, `BufferPoint`) address a position within a single source file, while **multibuffer coordinates** (`MultiBufferRow`, `MultiBufferOffset`, `MultiBufferPoint`) address a position within the unified scrollable view across all excerpts. Branded types enforce that these are never accidentally mixed.
...
The type of change a [DiffLine](#diffline) represents: `"equal"` for unchanged lines, `"insert"` for lines added in the new version (no `oldRow`), and `"delete"` for lines removed from the old version (no `newRow`).
