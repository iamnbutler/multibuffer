...
A pure function (`src/buffer/offset.ts`) that advances a `BufferOffset` through a chronological sequence of `EditEntry` values. Offsets before an edit pass unchanged; offsets after shift by `insertedLength − deletedLength`; offsets within the deleted range or at the edit start with `Bias.Left` clamp to the edit start; offsets at the edit start with `Bias.Right` jump past inserted text.
...
An automated PR triage agent (`.github/workflows/closer.md`). Runs after a review is submitted and labels PRs `ready-to-merge` (CI green, no blocking reviews), `needs-review` (blocking issues), or closes them (duplicate, spam, or fundamentally broken). Defaults to `needs-review` when uncertain. Chains naturally after the [Reviewer](#reviewer). Draft status is irrelevant — all PRs are triaged on their code and review state alone.
...
The specification for creating an excerpt: `context` is the full `BufferRange` to display (with surrounding context lines), and `primary` is the highlighted sub-range within `context`.
...
The behavior of `clipPoint` and `clipOffset` (`src/buffer/buffer.ts`) when a clamped position lands inside a UTF-16 surrogate pair (e.g., emoji or other supplementary Unicode characters outside the Basic Multilingual Plane). A position is inside a surrogate pair when it points at a low surrogate (code unit 0xDC00–0xDFFF); `Bias.Left` snaps back to the high surrogate (before the character); `Bias.Right` steps past the low surrogate (after it). This matches the surrogate-pair-aware cursor movement in `cursor.ts`.
...
An interface (`src/renderer/highlighter.ts`) describing a single incremental text edit to supply to tree-sitter. `startIndex`/`oldEndIndex`/`newEndIndex` are byte offsets of the changed range; `startPosition`/`oldEndPosition`/`newEndPosition` are the corresponding row/column positions. Passed alongside the new buffer text to `Highlighter.parseBuffer()` to enable [Incremental Parsing](#incremental-parsing). The helper `applyTreeEdit(tree, edit)` applies the descriptor to an existing tree before re-parsing.
