...
The project uses two distinct coordinate spaces: **buffer coordinates** address a row/column or byte offset within a single source file (`BufferRow`, `BufferOffset`, `BufferPoint`), while **multibuffer coordinates** address the unified scrollable view across all excerpts (`MultiBufferRow`, `MultiBufferOffset`, `MultiBufferPoint`). Branded types enforce that these are never accidentally mixed.
