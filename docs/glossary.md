...
A pure function (`src/buffer/offset.ts`) that advances a `BufferOffset` through a chronological sequence of `EditEntry` values. Applies `adjustOffsetSingle` for each edit in turn, respecting [Bias](#bias): offsets before the edit pass unchanged; offsets after shift by `insertedLength − deletedLength`; offsets at the edit start with `Bias.Right` jump past inserted text; with `Bias.Left` (or within the deleted range) they clamp to the edit start.
...
A hint controlling behavior at position boundaries — when text is inserted at an anchor's offset or a point is clipped to valid bounds. `Bias.Left` stays left of inserted text, clipping to before a boundary; `Bias.Right` advances past inserted text, clipping to at or after a boundary.
...
An automated PR triage agent (`.github/workflows/closer.md`). Runs after a review is submitted: applies `ready-to-merge` when CI is green with no blocking reviews, `needs-review` when blocking reviews or unresolved issues exist, and closes PRs that are duplicate, spam, or fundamentally broken. Defaults to `needs-review` when uncertain. Chains naturally after the [Reviewer](#reviewer). Draft status is irrelevant — all PRs are triaged on their code and review state alone.
...
The type of change a [DiffLine](#diffline) represents: `"equal"` (unchanged), `"insert"` (added in the new version, no `oldRow`), or `"delete"` (removed from the old version, no `newRow`).
...
The specification for creating an excerpt. Contains `context` (the full `BufferRange` to display, including surrounding context lines) and `primary` (the highlighted sub-range within `context`).
...
The behavior of `clipPoint` and `clipOffset` (`src/buffer/buffer.ts`) when a clamped position lands inside a UTF-16 surrogate pair (e.g., emoji or other supplementary Unicode characters outside the Basic Multilingual Plane). A position is inside a surrogate pair when it points at a low surrogate (code unit 0xDC00–0xDFFF); [Bias](#bias) then determines the snap direction: `Bias.Left` steps back to the high surrogate (before the supplementary character); `Bias.Right` steps past the low surrogate (after it).
...
An interface (`src/renderer/highlighter.ts`) describing a single incremental text edit to supply to tree-sitter. Carries byte-offset fields `startIndex`/`oldEndIndex`/`newEndIndex` and row/column position fields `startPosition`/`oldEndPosition`/`newEndPosition` for the changed range endpoints.
