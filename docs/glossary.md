...
A pure function (`src/buffer/offset.ts`) that advances a `BufferOffset` through a chronological sequence of `EditEntry` values. Applies `adjustOffsetSingle` for each edit in turn, respecting [Bias](#bias) to resolve ambiguous positions at edit boundaries: offsets before the edit pass through unchanged; offsets after the deleted range are shifted by `insertedLength − deletedLength`; offsets at the edit start with `Bias.Right` jump past inserted text, while those with `Bias.Left` (or within the deleted range) clamp to the edit start. Used by multibuffer anchor resolution when replaying edits since an anchor's recorded version.
...
An automated PR triage agent (`.github/workflows/closer.md`). Runs after a review is submitted and decides the outcome for each pull request: applies `ready-to-merge` when CI is green with no blocking reviews; applies `needs-review` when blocking reviews or unresolved issues exist; closes PRs that are duplicate, spam, or fundamentally broken. Defaults to `needs-review` when uncertain. Chains naturally after the [Reviewer](#reviewer). Draft status is irrelevant — all PRs are triaged on their code and review state alone.

See: `.github/workflows/closer.md`

### Coordinate Systems

The project uses two distinct coordinate spaces: **buffer coordinates** are row/column or byte offsets within a single source file (`BufferRow`, `BufferOffset`, `BufferPoint`), while **multibuffer coordinates** are positions within the unified scrollable view across all excerpts (`MultiBufferRow`, `MultiBufferOffset`, `MultiBufferPoint`). Branded types enforce that these are never accidentally mixed.
...
The specification for creating an excerpt. Contains `context` (the full `BufferRange` to display, including any surrounding context lines) and `primary` (the highlighted sub-range within `context`).
...
An automated adversarial code reviewer (`.github/workflows/reviewer.md`). Triggered on every PR event (opened, synchronize, ready_for_review) and via the `/review` slash command. Enforces the project's four priorities in order: accuracy, performance, consistency, public API UX. Uses `REQUEST_CHANGES` for blocking issues and `COMMENT` for non-blocking suggestions; hardballs every `biome-ignore` suppression (concrete justification required, or rewrite to avoid one); can sparingly file issues (max 1/run) for antipatterns recurring across multiple reviews; posts up to 25 inline review comments per run, prioritising blocking issues first. Read-only: never writes implementation code or pushes to branches.
...
The behavior of `clipPoint` and `clipOffset` (`src/buffer/buffer.ts`) when a clamped position lands inside a UTF-16 surrogate pair (e.g., emoji or other supplementary Unicode characters outside the Basic Multilingual Plane). A position is inside a surrogate pair when it points at a low surrogate (code unit 0xDC00–0xDFFF); [Bias](#bias) then determines the snap direction: `Bias.Left` steps back to the high surrogate (the position *before* the supplementary character), while `Bias.Right` steps past the low surrogate (the position *after* it). This matches the surrogate-pair-aware cursor movement in `cursor.ts`, which uses `codePointAt`/`prevCpStart` helpers to traverse pairs atomically.
