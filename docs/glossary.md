...
An automated PR triage agent (`.github/workflows/closer.md`). Runs after a review is submitted and decides each pull request's outcome: `ready-to-merge` when CI is green and no blocking reviews remain, `needs-review` when blocking reviews or unresolved issues exist, or closed if the PR is duplicate, spam, or fundamentally broken. Defaults to `needs-review` when uncertain. Chains naturally after the [Reviewer](#reviewer). Draft status is irrelevant — all PRs are triaged on their code and review state alone.
