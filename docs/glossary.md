...
A stable position within a buffer or multibuffer that survives text edits. Anchors track an offset and a [bias](#bias). Used to represent cursor positions and selection endpoints durably. See [Anchor Resolution](#anchor-resolution) for how an anchor is brought up to date.
...
A pure function (`src/buffer/offset.ts`) that advances a `BufferOffset` through a chronological sequence of `EditEntry` values, applying `adjustOffsetSingle` for each edit in turn. Offsets before the edit pass through unchanged; offsets after the deleted range shift by `insertedLength − deletedLength`. At the edit start, [Bias](#bias) resolves the ambiguity: `Bias.Right` jumps past the inserted text, while `Bias.Left` — or any offset within the deleted range — clamps to the edit start. Used by [Anchor Resolution](#anchor-resolution).

See: `src/buffer/offset.ts`, `src/multibuffer/anchor.ts`

### Auto-Indent

Inheriting the current line's leading whitespace on a newly created line. Implemented only by `insertLineBelow` and `insertLineAbove` (`src/editor/editor.ts`).

**`insertNewline` (Enter) does not auto-indent** — it inserts a bare `"\n"`. Note that in-code comments near the `insertLineBelow` / `insertLineAbove` implementations describe their behavior as matching "Enter auto-indent"; those comments are stale.
...
A mutable object representing a single file's text content, backed by a [rope](#rope). Buffers support `insert`, `delete`, and `replace` operations and maintain a monotonically increasing `version` counter. Each edit is appended to the [Edit Log](#edit-log).
...
A branded number type representing a **UTF-16 code unit** offset within a single buffer — not a byte offset. Distinct from [MultiBufferOffset](#multibufferoffset); see [Coordinate Systems](#coordinate-systems).
...
An immutable snapshot of a buffer's state, supporting the read-only operations `line`, `lines`, `text`, `pointToOffset`, `offsetToPoint`, and `clipPoint`. See [Snapshot Pattern](#snapshot-pattern).
...
An automated PR triage agent (`.github/workflows/closer.md`). Runs after a review is submitted and takes exactly one action per pull request: label `ready-to-merge` when CI is green and no blocking reviews remain, label `needs-review` when blocking reviews or unresolved issues exist, or close the PR outright if it is duplicate, spam, or fundamentally broken. Defaults to `needs-review` when uncertain. Chains naturally after the [Reviewer](#reviewer). Draft status is irrelevant — all PRs are triaged on their code and review state alone.

See: `.github/workflows/closer.md`

### Coordinate Systems

The project uses two distinct coordinate spaces, and branded types enforce that they are never accidentally mixed:
...
In both spaces, every offset and column is measured in **UTF-16 code units** (JavaScript `string.length`), never in bytes. The only byte-valued quantity in the codebase is [`TextSummary.bytes`](#textsummary), which is UTF-8 and is documented as display-only — never use it for position arithmetic.
...
An interface (`src/renderer/types.ts`) describing the full set of visual properties a [Decoration](#decoration) can apply to a row: `backgroundColor`, `color`, `borderColor`, `fontWeight`, `fontStyle`, `textDecoration`, and the gutter-specific `gutterBackground`, `gutterColor`, `gutterSign`, and `gutterSignColor`. The optional `isHunkSeparator` flag makes the gutter span the full width with no line numbers, used for hunk separator rows in the diff view. Decorations accept a `Partial<DecorationStyle>`, so any subset of fields may be specified.
...
Its methods are `reDiff()`, which recomputes immediately and returns the new `isEqual` state; `notifyChange()`, which schedules a debounced re-diff (`debounceMs`, default 150 ms); `onUpdate(callback)`, which subscribes to decoration updates and returns an unsubscribe function; and `dispose()`, which clears timers and subscriptions.
...
A per-buffer list of [EditEntry](#editentry) values recording every insert and delete since buffer creation. Exposed via `editsSince(version)` and consumed by [Anchor Resolution](#anchor-resolution).

### EditEntry

A single recorded buffer mutation: `{ offset, deletedLength, insertedLength }`. All values are in pre-edit buffer coordinates ([UTF-16 code units](#coordinate-systems)).
...
A branded [SlotKey](#slotkey) that uniquely identifies an excerpt. If an excerpt is removed and its slot reused, old `ExcerptId` values pointing to that slot are automatically invalid.
...
The specification for creating an excerpt: `context` is the full `BufferRange` to display including any surrounding context lines, and `primary` is the highlighted sub-range within it.
...
Note: the workflow says it delegates complex CI fixes by posting a `/pr-fix` comment, but no workflow currently handles that command, so the delegation is a no-op.
...
Fixed rendering constants: `lineHeight` and `gutterWidth`, plus the optional `charWidth` (auto-measured from the font by DOM renderers when omitted), `wrapWidth` (undefined or 0 disables [soft wrap](#soft-wrap)), and `gutterMode` — `"standard"` for a single line-number column, or `"diff"` for dual old/new columns plus a [gutter sign](#gutter-sign). All lines have the same height, enabling O(1) pixel↔row conversion.
...
A branded **UTF-16 code unit** offset within the multibuffer's unified view. Distinct from [BufferOffset](#bufferoffset).
...
An immutable snapshot of the multibuffer's state ([Snapshot Pattern](#snapshot-pattern)), supporting `lines`, `excerptAt`, `toBufferPoint`, `toMultiBufferPoint`, `resolveAnchor`, `resolveAnchors`, `clipPoint`, and `excerptBoundaries`. Carries a monotonically increasing `version` counter drawn from a module-level counter shared across all `MultiBuffer` instances, so versions are unique but not consecutive within one instance. The DOM renderer compares `version` to skip [WrapMap](#wrapmap) reconstruction when neither the snapshot content nor the wrap width has changed since the last render.
...
A mode in which the `Editor` silently ignores all text-mutating commands. Enabled via `readOnly: true` on the constructor or `editor.setReadOnly(true)` at runtime, and readable through the `editor.readOnly` getter. While active, `dispatch()` discards any command classified as an edit — `insertText`, `cut`, `redo`, `deleteLine`, `moveLine`, `duplicateLine`, `indentLines`, `dedentLines`, and others — while still processing cursor-movement commands.
...
It posts up to 25 inline comments per run with blocking issues first, using `REQUEST_CHANGES` for blocking findings and `COMMENT` for suggestions, and may sparingly file one issue per run for antipatterns recurring across reviews. Every `biome-ignore` suppression is hardballed: each must carry concrete justification or be rewritten to avoid needing one. Read-only — never writes implementation code or pushes to branches.

See: `.github/workflows/reviewer.md`

### Rope

The text storage structure backing each [buffer](#buffer). Splits text into chunks of at most 1024 UTF-16 code units, preferring newline boundaries; insert/delete/replace return new Rope instances with structural sharing of unchanged chunks. Caches chunk start offsets as a [prefix-sum](#prefix-sum) array for O(log n) line↔offset conversion.
...
A [generational arena](#generational-arena) (`src/multibuffer/slot_map.ts`) providing O(1) insert, remove, and lookup. Used to store excerpts and assign [ExcerptId](#excerptid) values.
...
Cached aggregate metrics for a span of text, stored per-excerpt to enable O(1) position lookups without scanning: `lines`, `lastLineLength` and `chars` (both in UTF-16 code units), and `bytes` — the UTF-8 encoded size, which is display-only and must not be used for position arithmetic. See [Coordinate Systems](#coordinate-systems).
