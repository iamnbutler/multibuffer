...
|---------|------------------|
| Indentation | `Tab` (with selection) / `Mod+]` indent, `Shift+Tab` / `Mod+[` dedent, auto-indent on Enter |
| Comment toggling | `Mod+/` (line comment) |
| Find & replace | `Mod+F` find, `Mod+G`/`F3` next, `Mod+Shift+G`/`Shift+F3` previous, `Mod+H` replace, `Mod+Shift+H` replace all |
| Multi-cursor | `Mod+D` next, `Mod+Shift+L` all, `Mod+Opt+Up/Down` add above/below, `Opt+Click` add at click, `Escape` collapse |
| Bracket pairs | Auto-close brackets/quotes; `Mod+Shift+\` jump to match |
| macOS text system | `Ctrl+A`/`E` line start/end, `Ctrl+K` kill to EOL, `Ctrl+Y` yank, `Ctrl+O` open line, `Ctrl+T` transpose |
