...
| Feature | Bindings |
|---------|---------|
| Indentation | `Tab` (indent), `Shift+Tab`/`Mod+[` (dedent), `Mod+]` (indent), auto-indent on Enter |
| Comment toggle | `Mod+/` |
| Find & replace | `Mod+F` (find), `Mod+G`/`F3` (next), `Mod+Shift+G`/`Shift+F3` (prev), `Mod+H` (replace), `Mod+Shift+H` (replace all) |
| Multi-cursor | `Mod+D` (next), `Mod+Shift+L` (all), `Mod+Opt+Up/Down` (above/below), `Opt+Click` (add), `Escape` (collapse) |
| Bracket pairs | Auto-close; `Mod+Shift+\` (matching bracket) |
| macOS text system | `Ctrl+A` (line start), `Ctrl+E` (line end), `Ctrl+K` (kill EOL), `Ctrl+Y` (yank), `Ctrl+O` (open line), `Ctrl+T` (transpose) |
| Scroll | `Mod+Opt+Up/Down` scroll without moving cursor |
