...
## State 08-24
main ce545ec (unmoved 155d). Same 9 TI PRs open (#312 #335 #357 #368 #538 #541 #543 #548 #690), ZERO human comments (#690's 5 are bot); #667 0/45 ticked, body 62.0K/65536 = NEXT RUN condense the 08-22 run entry into "Earlier runs" (did that to 08-21 this run). #611 open. #373=maintainer's, lint-blocked.
...
08-24 SRC BUG #7 EXCERPT-HEADER (#667 bug7/item40), unclaimed; #735=same expr DIFFERENT defect. editor-view:238==dom:760 IDENTICAL: boundaries queried by excerpt START, drawn at row-1 => excerpt starting AT endRow loses an ONSCREEN header. Sweep257 46->0. Item-30 3rd. DEAD API: RenderState.selections/.focused unread
...
UNAUDITED (by ratio, lowest first): injection-hl DONE 08-22 (2 bugs), diff/diff DONE 08-23 (bug#6), editor-view DONE 08-24 (bug#7; rest of the facade is thin wiring), diff-editor-view .75 (NEXT), diff/patch .92, diff/multi-file .95. hl-client DONE 08-21 (diff-client worker path still untested, same fake-Worker harness). Also still: cursor moveWord/movePage (movePage ignores wrapping - may be intended). project/glob.ts DONE 08-19.
