...
## State 08-29
main ce545ec (unmoved 160d). Same 9 TI PRs open (#312 #335 #357 #368 #538 #541 #543 #548 #690), ZERO human comments (#690's 5 = bot). #611 open (fixes the flake). #373=maintainer's, lint-blocked.
...
08-29 SRC BUG #11 VIEWPORT-ANCHORS, unclaimed, #667. resolveAnchorsInViewport (multibuffer.ts:374-384) filters by anchor's OLD excerpt but returns the RESOLVED one => ON-SCREEN anchor DROPPED (10/300 cases, 142 pairs; 0 after FIX=guard early-return on editsSince(anchor.ver).length>0). 🚨#664=SAME filter OPPOSITE dir; its fix CANNOT fix mine (filters only REMOVE). over-incl=BY DESIGN (doc+test:870): score under-incl ONLY. direct buf edit fires 0 textChange => SearchController stale.
...
UNAUDITED (by ratio): DONE = injection-hl 08-22, diff/diff 08-23, editor-view 08-24, diff-editor-view 08-25, diff/patch 08-26, multi-file 08-27, diff/controller 08-28, anchor+viewport-resolve 08-29, hl-client 08-21, glob 08-19. NEXT = diff/multibuffer or excerpt.ts. Also: diff-client worker path (fake-Worker harness); cursor moveWord/movePage (movePage ignores wrapping - may be intended).
