...
## State 09-05 (cmds revalidated: 2265/3/6/0, tc+lint clean)
main ce545ec (unmoved 167d). Same 9 TI PRs open (#312 #335 #357 #368 #538 #541 #543 #548 #690), 0 human comments. VERIFIED+urged: #745 #741 #748 #728(+#711) #743. #611 open.
...
09-04 editor.ts multi-sel EDIT paths AUDITED; editability bug ALREADY = PR #728 (rule-33 9th save). VERIFIED: 2272/0, revert-src = 5/7 its tests red, my 13 probes 13/13, +#711 = 2278/0 additive. 🔑 ENDPOINT-ONLY FIX INSUFFICIENT: locked MIDDLE excerpt still destroyed, need _edit's spanned LOOP. Endpoint checks added = 2265/0 IDENTICAL => 0/2274 distinguish; non-editable.test.ts = 20 tests ALL setCursor (item-14 8th). 🚨 PROBE NEEDS 1 SHARED BUFFER (MultiBuffer.edit declines cross-buffer).
09-03 input-handler AUDITED, both bugs = PRs #741 (Mod+Shift+K/Z dead; 64-combo sweep, exactly 2 change, 2277/0) + #748 (2284/0); CapsLock half UNPROVEN. Detail in #789.
...
09-05 hl-client+WORKER AUDITED. Blank-row cache bug = PR #743 (rule-33 10th save), VERIFIED w/ REAL worker: 1/10/10 reqs, revert-src 3/6 red, 2271/0. RESIDUE #743 declined = SRC BUG #15: _latestTokenRequests keyed by bufferId ONLY -> 5 concurrent DISJOINT getTokensAsync = 200/250 rows LOST (empty Map, uncached); DELETING guard = 2265/0 IDENTICAL (0/2274 cover); NOT post-parse protection (msgs IN ORDER; _inflightParses does it). 🔑 REAL-WORKER ORACLE: REAL *-worker.ts via `new URL(...,import.meta.url)` REPO-ROOT probe + playground/wasm/*.wasm; SUBCLASS global Worker to COUNT msgs.
...
UNAUDITED: DONE = glob, hl-client(+WORKER), injection-hl, diff/diff, editor-view, diff-editor-view, diff/patch, multi-file, diff/controller, anchor+viewport, excerpt, diff-client, selection, cursor, input-handler, editor._mergeSelections + multi-sel edit. NEXT = undo/redo (_applyInverse TOP-DOWN CORRECT 09-04; TESTED? #312); slot_map.set; highlighter REST (:95 done).
